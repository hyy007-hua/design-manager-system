...
public class TeacherServiceImpl  implements TeacherService{
    ...
    */
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public boolean resetPassword(String number ,String newPass) {
        int result = 0;
        try{
            result = teacherMapper.resetPassword(number,M5Util.md5encode(newPass));
        }catch (Exception e){
            e.printStackTrace();
        }
        if(result == 1)
    ...
    public void sendEmail(String account) {
        ...
        criteria.andEqualTo("number",account);
        TbTeacher teacher = teacherMapper.selectOneByExample(example);
        String code = SmallUtil.getRandomNum(6);
        Constant.codeMap.put(account, code);
        SendEmailUtil.sendVerificationCode(teacher.getEmail(),code);
    }
